...
protected:
	...
	void Ignore_Space_And_Comments(char* &LINE) {
		bool comment = false;
		bool space = false;

		do {
			comment = (*LINE == '(') ? true : false;
			if (comment) {
	...
	bool Read_Letter_And_Number(char* &LINE, char& CH, Number& N) {
		Ignore_Space_And_Comments(LINE);
		if (*LINE == 0 || *LINE < 'A' || *LINE > 'Z') return false;
...
public:
	...
	size_t Proccess_Gcode(char* &LINE);

	//GCodeParser()  { /*Traer de EEPROM la INFO! a  GcodeStatic */};
};
